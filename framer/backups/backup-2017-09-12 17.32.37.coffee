...
# certify.states.a =
# 	image: "images/certified_btn.png"
...
# 	print scroll.content.y
	for j in [0...7]
		if list_icons_images[j] == screenB.children[2].image
			listcurrentY = lists[j].y+scroll.content.y-40
...
for i in [0...7]
	...
	list.onTouchEnd (event, layer) ->
		...
		for n in [0...8]
			...
			coinbgs[n].scale = 0.4
			coinbgs[n].y = listcurrentY+80
			coinbgs[n].rotation = 0
			
		if backSwitch = true
			this.children[4].image = "images/list_btn_ed.png"
			avatar.image = this.children[2].image
